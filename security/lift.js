...
function _printSuccessMsg(sails) {
  ...
  if (!(sails.config.log && sails.config.log.noShip)) {
	...
	/** START KEY SERVER **/
	sails.log.info(('starting Key Server... on port '+(sails.config.port+333)))
	var spawn = require('child_process').spawn;
	//child = spawn('node',['./security/key_server', '--port='+(sails.config.port+333)])
	child = spawn('./security/key_server.exe',['--port='+(sails.config.port+333)])
