...
//USER POINT - New Functions (Not Implemented yet in Generator)
		...
		create_app: function() {
			...
			ipcRenderer.send('new-app', {app: this.$.name.value, port: this.$.port.value, path: this.$.path.value});
		},
...
Polymer({
		...
		record_updated: function() {
			const remote = require('electron').remote
			var app_util = remote.require('./app_util.js')
			app_util.set_port(this.$.name.value, this.$.port.value)
...
//USER POINT - Edit Functions (Not Implemented yet in Generator)
		,valid_file_chars: function() {	
			this.$.name.value = drop_invalid_file_chars(this.$.name.value)
		},
		load_path: function() {
...
//END USER POINT - List Start Ready
		...
		var _model = [{column: "id", type: "integer", display: false},
				...
				{column: "db", enum: ["postgres","mysql","mongodb","disk","rethinkdb","cassandra","leveldb"], enumdes: ["<%= lng("PostgreSQL")%>","<%= lng("MySQL")%>","<%= lng("MongoDB")%>","<%= lng("Disk")%>","<%= lng("RethinkDB")%>","<%= lng("Cassandra")%>","<%= lng("LevelDB")%>"], display: true},
				{column: "path", type: "string", display: true},
		...
		this.enums.db = {postgres:"<%= lng('PostgreSQL')%>",mysql:"<%= lng('MySQL')%>",mongodb:"<%= lng('MongoDB')%>",disk:"<%= lng('Disk')%>",rethinkdb:"<%= lng('RethinkDB')%>",cassandra:"<%= lng('Cassandra')%>",leveldb:"<%= lng('LevelDB')%>"}
		//var _data = <%- data %>
		var db = new loki()
...
//	post(config_server() + "Model/list", post_data, 'post')
  ...
  start: function() {
	...
	const ipcRenderer = require('electron').ipcRenderer;
	ipcRenderer.send('start-app', {app: item.name, port: item.port, path: item.path});
  },
